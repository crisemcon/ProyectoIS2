  ...
  const fetchSugerencias = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setDates(Object.entries(data.ContagiadosSalas).map((sala) => (sala[1].Personas.map((persona) =>( persona.Fecha_Termino )))));
      } else {
            ...
            {isLoading ? (
              ...
              <>
                <Grid item lg={12} sm={12} xl={12} xs={12}>
                  <SugerenciaSala nombre="Colegio" contagiados={contagiadosTotal} sugerencia={sugerencia} lastDate={null}/>
                </Grid>
                {
                  /*contagiadosSalas.map(sala => <Grid item lg={3} sm={6} xl={3} xs={12}>
                  <SugerenciaSala />
                </Grid>)*/
                  Object.entries(contagiadosSalas).map((sala, index) => (
                    <Grid key={sala[0]} item lg={3} sm={6} xl={3} xs={12}>
                      <SugerenciaSala nombre={sala[0]} contagiados={sala[1].Contagiados} sugerencia={sala[1].Sugerencia} lastDate={dates[index].length !== 0 ? new Date(Math.max(...dates[index].map(e => new Date(e)))) : null}/>
                    </Grid>
                  ))
                }
              </>
